...
class BarangkeluarController extends Controller
{
    ...
    public function print(Request $request){
        ...
        $data['data'] = BarangkeluarModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangkeluar.barang_kode')->whereIn('bk_id', $id)->orderBy('bk_id', 'DESC')->get();
        return view('Admin.BarangKeluar.print', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('checkbox', function ($row) {
                    $checbox = '<input class="" name="idbk" type="checkbox" value="' . $row->bk_id . '" />';
                ...
                ->addColumn('harga', function ($row) {
                    $harga = 'Rp ' . number_format($row->bk_harga, 0);

                    return $harga;
                })
                ->addColumn('total', function ($row) {
                    $total = 'Rp ' . number_format($row->bk_total, 0);

                    return $total;
                })
                ->addColumn('statuspengiriman', function ($row) {
                    if ($row->bk_status == 'pending') {
                        $status = '<span class="badge bg-dark">' . $row->bk_status . '</span>';
                    } else if ($row->bk_status == 'terkirim') {
                        $status = '<span class="badge bg-primary">' . $row->bk_status . '</span>';
                    } else if ($row->bk_status == 'selesai') {
                        $status = '<span class="badge bg-success">' . $row->bk_status . '</span>';
                    } else {
                        $status = '-';
                    }
                ...
                ->addColumn('action', function ($row) {
                    $array = array(
                        ...
                        "bk_harga" => $row->bk_harga,
                        "bk_total" => $row->bk_total,
                    );
                    $button = '';
                ...
                ->rawColumns(['action', 'tgl', 'tujuan', 'barang', 'statuspengiriman', 'harga', 'total', 'checkbox'])->make(true);
        }
    }
    ...
    public function proses_tambah(Request $request)
    {
        ...
        BarangkeluarModel::create([
            ...
            'bk_status'   => $request->pengiriman,
            'bk_harga'   => $request->harga,
            'bk_total'   => $request->total,
        ]);
